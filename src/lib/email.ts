...
export async function sendMail(input: SendInput) {
  const from = envVar('EMAIL_FROM') ?? 'Polinomics <auth@polinomics.local>';
...
export function magicLinkEmail(link: string, purpose: 'login' | 'register') {
  ...
  const subject = purpose === 'register' ? 'Bienvenido a Polinomics' : 'Tu link de acceso a Polinomics';
  const text = `Hacé click en este link para ${action} en Polinomics (vence en 15 minutos):
...
<html><body style="font-family: ui-monospace, Menlo, Consolas, monospace; max-width: 520px; margin: 40px auto; padding: 0 24px; color: #0a0a0a;">
  <p style="font-size: 12px; letter-spacing: 0.18em; text-transform: uppercase; color: #6b6b6b;">// polinomics · auth</p>
